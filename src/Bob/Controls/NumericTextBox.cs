...
namespace Bob.Controls
{
    public class NumericTextBox : TextBox
    {
        ...
        private static readonly Regex DecimalRegex = new Regex(@"(?n:(^(?!0,?\d)\d{1,3}(?=(?<1>,)|(?<1>))(\k<1>\d{3})*(\.\d\d\d\d)?)$)");
        ...
        void LostFocusHandler(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (!regex.IsMatch(Text))
                {
                    ...
                    var amount = Math.Truncate(decimal.Parse(sanatizedInput)*10000)/10000;

                    Text = amount.ToString(); //.PadLeft(sanatizedInput.Length, '0');
                }
                else
                {
                    var amount = Math.Truncate(decimal.Parse(Text)*10000)/10000;
